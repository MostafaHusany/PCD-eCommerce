<div class="text-center">
    @if(auth()->user()->hasRole('admin') || auth()->user()->isAbleTo(['user_show']))
    <button class="show-object btn btn-xs btn-info" data-object-id="{{$row_object->id}}">
        <i class="fas fa-eye"></i>
    </button>
    @endif
    
    @if(auth()->user()->hasRole('admin') || auth()->user()->isAbleTo(['user_edit']))
    ...
    @if(auth()->user()->hasRole('admin') || auth()->user()->isAbleTo(['user_delete']))
